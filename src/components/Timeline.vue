  ...
  <div>
    ...
    <div class="flex flex-col px-3">
      <div class="incidents-timeline" v-for="(day, i) in timeline" :key="i">
        ...
        <div class="flex flex-col p-4 rounded-md bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-500 shadow" v-if="day.incidents.length !== 0">
          <div class="incident" v-for="(incident, i) in day.incidents" :key="i">
            <div class="flex justify-between">
              <span class="flex text-lg mb-2 font-medium">
                <span class="mr-2">{{ incident.name }}</span>
            ...
            <div class="pb-4 message">
              <div class="pb-2" v-for="(activity, i ) in incident.activities" :key="i">
                <span class="font-bold">{{ activity.status }} - </span>
                <span>{{ activity.message }}</span>
                <div class="date">{{ parseDate(activity.timestamp) }} UTC</div>
              </div>
            </div>
          </div>
        </div>
        <span class="no-message" v-else>No incidents reported</span>
      </div>
    </div>
  </div>
</template>
